...
const Chatbot: React.FC<ChatbotProps> = ({ onOpenForgotPassword }) => {
  ...
  const [dangKyHoatDongList, setDangKyHoatDongList] = useState<DangKyHoatDong[]>([]);
  ...
  useEffect(() => {
    const fetchDangKyHoatDong = async () => {
      if (maSV) {
        try {
          const token = localStorage.getItem("token");
          if (!token) {
            console.warn("Không tìm thấy token để gọi API danh-sach-dang-ky");
            return;
          }
          const response = await axios.get(`http://localhost:5163/api/DangKyHoatDongs/danh-sach-dang-ky?maSV=${maSV}`, {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
            },
          });
          // API trả về { message: "...", data: [...] }, lấy mảng từ response.data.data
          const data = Array.isArray(response.data.data) ? response.data.data : [];
          setDangKyHoatDongList(data);
        } catch (error: any) {
          console.error("Lỗi khi lấy danh sách hoạt động đã đăng ký:", {
            message: error.message,
            response: error.response?.data,
          });
          setDangKyHoatDongList([]);
        }
      }
    };
    fetchDangKyHoatDong();
  ...
  useEffect(() => {
    const initialMessage: ChatMessage = {
      text: studentInfo?.HoTen
        ? `Xin chào ${studentInfo.HoTen}! Tôi là trợ lý ảo HUIT. Tôi có thể giúp gì cho bạn? (Bạn có thể xem thông tin cá nhân, chỉnh sửa thông tin, phản hồi điểm rèn luyện, đăng ký hoạt động, xem danh sách hoạt động đã đăng ký, xem điểm, xem thông báo, hoặc nếu quên mật khẩu, tôi cũng có thể giúp.)`
  ...
  // Lấy danh sách hoạt động mở đăng ký
  const fetchHoatDong = async () => {
    try {
      ...
      setHoatDongList(data);
      const activeHoatDong = data.filter(
        (hd: HoatDong) => hd.TrangThai === "Đang diễn ra" || hd.TrangThai === "Đang mở đăng ký"
      );
      const registeredHoatDongIds = dangKyHoatDongList.map((dk) => dk.MaHoatDong);
      const availableHoatDong = activeHoatDong.filter((hd: HoatDong) => !registeredHoatDongIds.includes(hd.MaHoatDong));

      if (availableHoatDong.length === 0) {
        return "Hiện không có hoạt động nào đang mở đăng ký hoặc bạn đã đăng ký tất cả.";
      }
      let responseText = "Các hoạt động đang mở đăng ký mà bạn chưa tham gia:\n\n";
      availableHoatDong.forEach((hd: HoatDong, index: number) => {
  ...
  const handleDangKyHoatDong = async (index: number) => {
    try {
      ...
      if (index < 1 || index > availableHoatDong.length) {
        return "Số thứ tự không hợp lệ. Vui lòng chọn lại.";
      }
      const selectedHoatDong = availableHoatDong[index - 1];
      ...
      const response = await axios.post(
        ...
        { MaHoatDong: Number(selectedHoatDong.MaHoatDong) },
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      // Cập nhật lại danh sách đã đăng ký
      const updatedDangKy = await axios.get(`http://localhost:5163/api/DangKyHoatDongs/danh-sach-dang-ky?maSV=${maSV}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      });
      const updatedData = Array.isArray(updatedDangKy.data.data) ? updatedDangKy.data.data : [];
      setDangKyHoatDongList(updatedData);
  ...
  const handleChatSend = async () => {
    ...
    let responseText = "Tôi chưa hiểu yêu cầu của bạn. Hãy thử: 'xem thông tin', 'chỉnh sửa thông tin', 'phản hồi điểm', 'đăng ký hoạt động', 'xem danh sách hoạt động đã đăng ký', 'xem điểm', 'xem thông báo', 'thời tiết [địa điểm]', hoặc 'quên mật khẩu'.";
      ...
      if (waitingForActivityConfirmation !== null) {
        ...
        if (!isNaN(index)) {
          ...
          responseText = "Vui lòng nhập số thứ tự hợp lệ.";
            ...
            const thongTinMatch = chatInput.match(/(xem thông tin|thông tin cá nhân|profile|thông tin của tôi|hiển thị thông tin|hồ sơ cá nhân|kiểm tra thông tin)/i);
        const chinhSuaMatch = chatInput.match(/(chỉnh sửa thông tin|cập nhật thông tin|sửa thông tin|thay đổi thông tin|điều chỉnh thông tin|cải tiến thông tin)/i);
        const phanHoiMatch = chatInput.match(/(phản hồi |khiếu nại điểm rèn luyện|phản hồi điểm rèn luyện|gửi phản hồi|điểm rèn luyện khiếu nại|phàn nàn điểm)/i);
        const dangKyMatch = chatInput.match(/(đăng ký hoạt động|tham gia sự kiện| tham gia|đăng ký sự kiện|tham gia hoạt động|ghi danh hoạt động)/i);
        const xemDangKyMatch = chatInput.match(/(xem danh sách hoạt động đã đăng ký|hoạt động đã đăng ký của tôi|danh sách đăng ký|kiểm tra hoạt động đã đăng ký|xem lịch sử đăng ký)/i);
        const diemMatch = chatInput.match(/(xem điểm\s*(?:DRL\s*)?(?:của\s*)?(?:MSSV\s*)?|điểm\s*(?:rèn luyện|DRL)\s*(?:kỳ này\s*)?(?:của\s*)?(?:tôi|MSSV\s*)?|tôi được bao nhiêu điểm|MSSV\s*(\w+)\s*điểm\s*(?:luyện|rèn luyện)\s*bao nhiêu|điểm số của tôi|kiểm tra điểm)(\w+)?/i);
        const hocKyMatch = chatInput.match(/ kỳ\s*(\d+)|học kỳ\s*(\d+)/i);
        const mssvMatch = chatInput.match(/DHTH\d+|mã sinh viên\s*(DHTH\d+)/i);
        const hoatDongMatch = chatInput.toLowerCase().includes("hoạt động") && !dangKyMatch && !xemDangKyMatch;
        ...
        if (thongTinMatch) {
          ...
          if (!maSV) {
            ...
            responseText = await fetchHoatDong();
              ...
              } else if (xemDangKyMatch) {
        if (!maSV) {
          responseText = "Vui lòng đăng nhập để xem danh sách hoạt động đã đăng ký.";
        } else if (dangKyHoatDongList.length === 0) {
          responseText = "Bạn chưa đăng ký bất kỳ hoạt động nào.";
        } else {
          responseText = `Danh sách hoạt động đã đăng ký (${dangKyHoatDongList.length} hoạt động):\n\n`;
          dangKyHoatDongList.forEach((dk: DangKyHoatDong, index: number) => {
            responseText += `${index + 1}. Tên hoạt động: ${dk.TenHoatDong}\n   Ngày bắt đầu: ${new Date(dk.NgayBatDau).toLocaleString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}\n Địa điểm : ${dk.diaDiem} \n  Mô tả: ${dk.MoTa}\n   Điểm cộng: ${dk.diemCong} điểm\n   Ngày đăng ký: ${new Date(dk.NgayDangKy).toLocaleString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}\n\n`;
          });
        }
